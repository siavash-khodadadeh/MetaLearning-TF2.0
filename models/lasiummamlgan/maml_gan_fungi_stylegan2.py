...
from databases import MiniImagenetDatabase, FungiDatabase
...
class MiniImageNetMAMLStyleGan2(MAMLGAN):
    ...
    def get_images_from_vectors(self, vectors):
        ...
        n2 = np.random.uniform(0.0, 1.0, size=[vectors.shape[0], 64, 64, 1]).astype('float32')
        # n2 = np.random.normal(size=vectors.shape[0] * 64 * 64 * 1, scale=0.1).astype('float32').reshape(
        #     [vectors.shape[0], 64, 64, 1]
        # )
        # trunc = 1.0
...
if __name__ == '__main__':
    ...
    maml_gan = MiniImageNetMAMLStyleGan2(
        ...
        num_tasks_val=100,
        clip_gradients=True,
        experiment_name='fungi_stylegan2_23_p1_0.25_aug_fo',
        val_seed=42,
        val_test_batch_norm_momentum=0.0,
        first_order=True
    )

    print(maml_gan.experiment_name)
