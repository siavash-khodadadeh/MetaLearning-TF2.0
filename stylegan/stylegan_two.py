...
def upsample_to_size(x, im_size=64):
    y = im_size // x.shape[2]
...
class GAN(object):
    ...
    def generator(self):
        ...
        # Edited for size 64 (In original implementation the following lines are not commented.)
...
class StyleGAN(object):
    ...
    def evaluate(self, num = 0, trunc = 1.0):
        ...
        x.save(os.path.join(
            ...
            "stylegan/Results/i"+str(num)+".png"
        ))
        ...
        x.save(os.path.join(
            settings.PROJECT_ROOT_ADDRESS,
            "stylegan/Results/i"+str(num)+"-ema.png"
        ))
        ...
        x.save(os.path.join(
            settings.PROJECT_ROOT_ADDRESS,
            "stylegan/Results/i"+str(num)+"-mr.png"
        ))
    ...
    def generateTruncated(self, style, noi = np.zeros([44]), trunc = 0.5, outImage = False, num = 0):
        ...
        if outImage:
            ...
            x.save(os.path.join(
                settings.PROJECT_ROOT_ADDRESS,
                "stylegan/Results/t"+str(num)+".png"
            ))
    ...
    def saveModel(self, model, name, num):
        ...
        with open(os.path.join(
                settings.PROJECT_ROOT_ADDRESS,
                "stylegan/pretrained_models/"+name+".json"), "w") as json_file:
            json_file.write(json)

        model.save_weights(os.path.join(
            settings.PROJECT_ROOT_ADDRESS,
            "stylegan/pretrained_models/"+name+"_"+str(num)+".h5"
        ))
    ...
    def loadModel(self, name, num):
        ...
        file = open(os.path.join(
            settings.PROJECT_ROOT_ADDRESS,
            "stylegan/pretrained_models/"+name+".json"
        ), 'r')
        json = file.read()
        file.close()

        mod = model_from_json(json, custom_objects = {'Conv2DMod': Conv2DMod})
        mod.load_weights(os.path.join(
            settings.PROJECT_ROOT_ADDRESS,
            "stylegan/pretrained_models/"+name+"_"+str(num)+".h5"
        ))
